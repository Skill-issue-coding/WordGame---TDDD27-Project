...
const GameModeCard = ({ title, description, icon, players, color, onClick }: GameModeCardProps) => {
  return (
    <button
      onClick={onClick}
      className={cn(
        "game-card border-2 text-left transition-all duration-300 hover:scale-[1.02] cursor-pointer group w-full",
        colorMap[color],
      )}>
      <div className="flex items-start gap-4">
        <span className="text-4xl">{icon}</span>
        <div className="flex-1 min-w-0">
          <h3 className={cn("font-display text-xl font-bold mb-1", textColorMap[color])}>
            {title}
          </h3>
