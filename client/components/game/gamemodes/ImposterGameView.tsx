...
export const ImpostorView = () => {
  ...
  // Format players from the lobbyState
  const players = useMemo(() => {
    if (!lobbyState?.users) return [];
    return Object.entries(lobbyState.users).map(([id, p]) => ({
      id,
      name: p.username,
      color: p.background || "blue", // Fallback color
    }));
  }, [lobbyState?.users]);

  const impostorCount = lobbyState?.settings && "impostor_count" in lobbyState.settings ? lobbyState.settings.impostor_count : 1;

  // Determine if the current user is an impostor
  const isImpostor = useMemo(() => {
    if (!user?.user_id || players.length === 0) return false;

    // IDEAL PRODUCTION WAY: The server decides and sends an array of impostor IDs
    // return lobbyState.impostors?.includes(user.user_id);

    // DEVELOPMENT FALLBACK: Deterministically pick impostors on the frontend.
    // By sorting the IDs, all clients will independently pick the exact same impostors.
    const sortedIds = [...players].map((p) => p.id).sort();
    const impostorIds = sortedIds.slice(0, impostorCount);

    return impostorIds.includes(user.user_id);
  }, [players, impostorCount, user?.user_id]);
