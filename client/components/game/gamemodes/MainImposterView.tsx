...
export const MainImpostorView = () => {
  ...
  const readyTime = game?.phaseState || game?.roundState ? (game?.phaseState?.timers?.ready_time ?? game?.roundState?.timers?.ready_time) : fakePhaseState.timers.ready_time;
  ...
  const phase = game && game.phaseState ? game.phaseState.game_phase : fakePhaseState.game_phase;
