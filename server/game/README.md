...
|---|---|---|
| 1 | `events.go:56` | `game_started` has no payload (`TODO: Define payload`). Currently `nil`; clients infer game start from `impostor_new_round`. |
| 2 | `payloads.go:39` | `ImpostorIntermediateStatePayload` is defined but never sent. Either wire it up or remove it. |
| 3 | `lobby.go:151` | `ModeContextoBattle` and `ModeSynonymDuel` fall through to "Spelläget stöds inte än" — games not wired up yet. |
| 4 | `impostor.go:404` | `previousRoundSubmissions` only includes players who submitted at least once; players who skipped input are absent. Frontend must handle missing keys. |
| 5 | `impostor.go` | No per-player vote acknowledgment — after casting a vote the voter receives no confirmation event. |
